...
foreach (string line in secondSection)
{
	...
	if (!isCorrectUpdate)
	{
		List<int> correctedUpdate = FixOrder(updateNumbers);
		result += correctedUpdate[updateNumbers.Length / 2];
	}
}

Console.WriteLine(result);

List<int> FixOrder(int[] incorrectOrderNumbers)
{
	List<int> newOrder = new List<int>();
	foreach (int incorrectOrderNumber in incorrectOrderNumbers)
	{
		for (int i = 0; i < newOrder.Count; i++)
		{
			if (rules[incorrectOrderNumber].Contains(newOrder[i]))
			{
				newOrder.Insert(i,incorrectOrderNumber);
				break;
			}
		}


		if (!newOrder.Contains(incorrectOrderNumber))
		{
			newOrder.Add(incorrectOrderNumber);
		}
	}
